        ...
        sheet("Schedule") {
            ...
            WeekDay.entries.take(6).forEach { day ->
                (0..6).forEach { classNum ->
                    row {
                        if (classNum == 0) {
                            cell(dayOfWeekToString(day))
                        } else {
                            cell("")
                        }
                        cell(classNumberToHoursString(classNum))
...
private fun dayOfWeekToString(day: WeekDay): String {
    return when (day) {
...
private fun classNumberToHoursString(classNumber: Int): String {
